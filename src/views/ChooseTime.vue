<template>
	<div class="container container-flex container-flex-row container-wrap">
		<button class="button chooseTime-button" v-for="(time, index) in times" :key="index">
			{{ ('0' + time.startingTime.hours).slice(-2) }} : {{ ('0' + time.startingTime.minutes).slice(-2) }} ~ {{ ('0' + time.endingTime.hours).slice(-2) }} : {{ ('0' + time.endingTime.minutes).slice(-2) }}
		</button>
	</div>
</template>
	...
	computed: {
		times: function() {
			...
			while (!isOutOfBound(endingTime, maxTime)) {
				t.push({startingTime: {...startingTime}, endingTime: {...endingTime}});
